% hw3_ex5_whsolo_sorhom.m
% Will Solow and Skye Rhomberg
% CS346 - Spring 2020
% HW3 - HH simulations

% This code simulates the HH Model with the concentration gated Na-K pump
% and voltage gated leakage channel, graphing V, m,
...
P_On = 1;
% used to turn the leakage channel on and off. 1 is open, 0 is closed.
% Starts as open as initial voltage is less than -54.5mV
L_On = 1;
...
V_init = -65; % resting/action potential (mV)
...
% calculating rate of change of voltage based on currents. Incorporates the
% voltage gating for pump, ion channels, and leakage
dVdt = @(V, n, m, h, I, Na_On, K_On, P_On, L_On) (I - K_On*I_K(V,n) - ...
    Na_On*I_Na(V,m,h) - L_On*I_L(V) + P_On*I_P) / C_M;
...
dKdt = @(V, n, K_On, P_On, L_On) -I_K(V,n)*K_On - L_On*I_L(V) - 2*P_On*I_P;
...
for i=1:num_iterations
    ...
    dK1 = dKdt(V(i)      , n(i)                  , K_On, P_On, L_On)*dt;
    ...
    dK2 = dKdt(V(i)+dV1/2, n(i)+dN1/2            , K_On, P_On, L_On)*dt;
    ...
    dK3 = dKdt(V(i)+dV2/2, n(i)+dN2/2            , K_On, P_On, L_On)*dt;
    ...
    dK4 = dKdt(V(i)+dV3  , n(i)+dN3              , K_On, P_On, L_On)*dt;
    ...
    % open/close leakage channel. Open if voltage is less than L_close
    % (typically -54.4mV)
    L_On = (V(i+1) < L_close);
end
